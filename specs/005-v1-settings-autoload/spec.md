...
- **FR-013**: Nomes MUST ser idênticos ao GDScript: propriedade `config_file`; métodos `load_settings`, `save_settings`, `apply_graphics_settings`; ação `toggle_fullscreen` tratada em `_input`. Conferidos contra os 10 sítios de acesso dos 5 módulos Rust.
...
- **Settings (autoload, `/root/Settings`)**: propriedade exposta `config_file`; métodos expostos `load_settings()`, `save_settings()`, `apply_graphics_settings(window, environment, scene_root)`; estado `metalfx_supported`; padrões (15 chaves em 2 seções); trata `toggle_fullscreen`. Consumido dinamicamente por 5 módulos Rust (10 sítios).
