...
- [x] T002 Registrar baseline do build: `cd oxide_godot_core && cargo build 2>&1 | grep -c '^warning'` → esperado `0`; anotar em `specs/005-v1-settings-autoload/quickstart.md` §1 se diferir
- [x] T003 Registrar baseline do import: `cd oxide-godot && /usr/bin/godot.x86_64 --headless --import --path . 2>&1 | tee /tmp/import.log`; confirmar `grep -c 'Initialize godot-rust' /tmp/import.log` = 1 e `grep -E 'ERROR|SCRIPT ERROR' /tmp/import.log | grep -vE 'Cannon_Charge already exists|doorsimple_d.png|surfaces.is_empty'` vazio
