...
| 3 | `part + red_robot: bridges, part phase machine on the frame schedule, robot seven-set tick with same-run hit (RobotHitLocal), robot AnimationTree MANUAL + advance (R1 option B)` | gates 214; headless; harness (a), (b), (d), (e) both trees; **STOP 1** single player; **STOP 2** multiplayer |
...
| `sync_out_robot` writes another entity's component (`PartPhase::Waiting` through `EntityIndex`, R5) | The constitution's `SyncOut` is "components → engine"; the part's phase must change in the same run as the robot's death (option (B), spec US2 scenario 2, user-approved) | Cross-entity component write from a `SyncOut` system, recorded as a spec-sanctioned exception; `CLAUDE.md` names it (commit 4) |
