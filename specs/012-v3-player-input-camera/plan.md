...
| `jump`/`land`/`shoot` on `Player`: `call_local` → `call_remote` (FR-004, option (b), user-approved at plan review 2026-09-18) | A locally-invoked handler could only push an event consumed by the FRAME run: `FireCooldown` would start one frame late (the `Timer` node processes before the driver), shoot/hit trauma would land after `shake_decide`, and R2's transient animation write could be sampled by replication. In v2 all local effects ran inline in the physics step | The `Simulates` entity's fixed `SyncOut` applies the local effects itself in v2's order; the RPC reaches remote peers only, whose handlers push `PlayerFx`. Names/signatures unchanged; the attribute is the one deviation, listed in `docs/v3-tradeoffs.md`. Harness (b)/(c) RAW stamps prove same-step timing |
| `Handles::Player.root` is `Gd<Player>` (user class), not the upcast `CharacterBody3D` | The projection writes of `motion`/`current_animation` are `#[var]` fields of the bridge, reachable only through `bind_mut()`; a `bind_mut()` held across `rpc`/`play`/`start` would double-borrow when a callback fires | Glue rule in `sync_out_player`: the guard is dropped before any engine call that can invoke a callback; recorded in `contracts/player-entity.md` |
