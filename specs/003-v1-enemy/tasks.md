...
- **EnemyRobot**: enum `State { Idle, Approach, Aim, Shooting }` com
  `#[derive(GodotConvert, Var, Export, Clone, Copy, PartialEq, Debug)] #[godot(via = i64)]` (D7);
  `#[signal] fn exploded();` no bloco `#[godot_api] impl EnemyRobot` **único**, emissão
...
**Goal**: `enemies/red_robot/red_robot.gd` (283 l.) → `EnemyRobot: CharacterBody3D`; sinal
...
Phase 1 (Setup: T001–T005)          — baseline de red_robot.tscn e level.tscn
  ...
  → Phase 4 US2 (T016–T028)  → commit 2  (EnemyRobot; red_robot.tscn: raiz + ext_resource id="1"; player.rs pub(crate))
